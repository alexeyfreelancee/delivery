...
class SupplierFreshFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_supplier_fresh, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        view.findViewById<RecyclerView>(R.id.supplierFreshList).adapter = adapter

        viewModel.supplierOrdersFresh.observe(viewLifecycleOwner,{
            adapter?.updateList(it)
        })
        CoroutineScope(Dispatchers.IO).launch {
            viewModel.updateSupplierFresh()
        }
